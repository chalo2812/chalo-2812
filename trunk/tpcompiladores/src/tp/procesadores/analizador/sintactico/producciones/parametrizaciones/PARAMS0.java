...
public class PARAMS0 extends Produccion {
   ...
   public boolean reconocer(LexicAnalyzer lexic, TokensVisitor visitor, SintacticAnalyzer sintactic, List<Parametro> parametrosH,
                            ListaParametrosHandler listaS) {
      boolean reconoce;
      ParametroHandler parametroSp1 = new ParametroHandler();
      producciones.set(0, new TIPOPARAM0());
      reconoce = producciones.get(0).reconocer(lexic, visitor, sintactic, new Parametro(), parametroSp1);
      if (sintactic.siguiente.getClass() == Palabra.class) {
         ParametroHandler parametroSp2 = new ParametroHandler();
         reconoce = producciones.get(1).reconocer(lexic, visitor, sintactic, parametroSp1.getParametro(), parametroSp2);
         if (reconoce) {
            ...
            if (reconoce) {
               ...
               reconoce = producciones.get(3).reconocer(lexic, visitor, sintactic, parametroSp2.getParametro(), parametroSp3);
               if (reconoce) {
                  ...
                  reconoce = producciones.get(4).reconocer(lexic, visitor, sintactic, parametrosH, listaSp);
